...
class CustomDataset(Dataset):
    def __init__(self, img, txt, ans):
        self.img = img
        # txt Series ise index’i sıfırla; değilse listeye çevir
        if isinstance(txt, pd.Series):
            self.txt = txt.reset_index(drop=True)
        else:
            self.txt = list(txt)
        self.ans = ans
    ...
    def __getitem__(self, idx):
        img = self.img[idx]
        # Series ise iloc, değilse normal index
        if isinstance(self.txt, pd.Series):
            txt = self.txt.iloc[idx]
        else:
            txt = self.txt[idx]
        ans = self.ans[idx]
        return img, txt, ans
...
class CLIPAligner(nn.Module):
    def __init__(self, image_encoder, text_encoder_head, proj_dim=256):
        super().__init__()
        self.image_encoder = image_encoder
        self.text_encoder_head = text_encoder_head
        self.img_proj = nn.Linear(128, proj_dim)
        self.txt_proj = nn.Linear(128, proj_dim)

    def forward(self, images, text_tokens):
        img = self.image_encoder(images)
        txt = self.text_encoder_head(text_tokens["input_ids"], text_tokens["attention_mask"])
...
def contrastive_loss(img, txt, temperature=0.07):
        ...
        return (Fnn.cross_entropy(logits, labels) + Fnn.cross_entropy(logits.t(), labels)) / 2    
    
def contrastive_train(model, dataloader, optimizer, device):
        model.train()
        total_loss = 0
...
def train_validate_pipeline(text_encoder_name, image_encoder_name, device):
    ...
    result_path = f"models_unfreeze_{num_of_unfreeze_block}block_con_pre_lr\{text_encoder_name}_{image_encoder_name}.pth"
    ...
    # QUICK TEST MODE ✅
    # =====================
    quick_test = False  # hızlı küçük deneme için

    if quick_test:
        train_df = train_df.sample(200, random_state=42)
        val_df = val_df.sample(80, random_state=42)
        test_df = test_df.sample(80, random_state=42)

        print("✅ Quick Test MODE: Enabled (200 train / 80 val / 80 test)")
    ...
    print(">>>> Contrastive Pretraining Started <<<<")
    clip_model = CLIPAligner(image_encoder, text_head).to(device)
    optimizer_c = torch.optim.AdamW(filter(lambda p: p.requires_grad, clip_model.parameters()), lr=1e-5)

    for e in range(5):  # 5 epoch yeterli
        c_loss = contrastive_train(clip_model, trainloader, optimizer_c, device)
        print(f"[Contrastive Epoch {e+1}] loss={c_loss:.4f}")

    # contrastive sonrası aligned encoderları alıyoruz
    image_encoder = clip_model.image_encoder
    text_head = clip_model.text_encoder_head
    ...
    for epoch in range(epochs):
        ...
        if valid_acc > best_acc:
            ...
            if text_encoder_name == 'ELMo':
                ...
                torch.save(model.state_dict(), result_path, _use_new_zipfile_serialization=False)
    ...
    with open(f"results_unfreeze_{num_of_unfreeze_block}block_con_pre_lr/classification_report_{text_encoder_name}_{image_encoder_name}.txt", "w", encoding="utf-8") as f:
        f.write(report_str)
...
if __name__ == '__main__':
    ...
    os.makedirs(f"results_unfreeze_{num_of_unfreeze_block}block_con_pre_lr", exist_ok=True)
    os.makedirs(f"models_unfreeze_{num_of_unfreeze_block}block_con_pre_lr", exist_ok=True)
    ...
    for ie in image_encoders:# allahümme ya şafi jfsjdj
        for te in text_encoders:
            ...
            result_path = f"models_unfreeze_{num_of_unfreeze_block}block_con_pre_lr/{te}_{ie}.pth"
            ...
            try:
                ...
                if "does not match size of target_names" in str(e):
                    ...
                    try:
                        ...
                        with open(f"results_unfreeze_{num_of_unfreeze_block}block_con_pre_lr/classification_report_{te}_{ie}_partial.txt", "w", encoding="utf-8") as f:
                            f.write(report_str)
    ...
    print("\nFinal Results:\n", df_results)
    df_results.to_excel(f"results_unfreeze_{num_of_unfreeze_block}block_con_pre_lr/test_results.xlsx", index=False)
